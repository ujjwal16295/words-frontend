...
export default function AllWordsPage() {
  ...
  const fetchAllWords = async (page) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE}/api/vocabulary?page=${page}&limit=50`
      );
  ...
  const handleLoadMore = () => fetchAllWords(pagination.page + 1);
  ...
  const handleJumpToPage = async () => {
    ...
    try {
      ...
      scrollToTop();
      const response = await fetch(
        `${API_BASE}/api/vocabulary?page=${pageNum}&limit=50`
      );
      const result = await response.json();
  ...
  const handleToggleStar = async (word, currentStarred) => {
    ...
    // Optimistic update
    setWords((prev) =>
      prev.map((w) => (w.word === word ? { ...w, is_starred: !currentStarred } : w))
    );
    try {
        ...
        <div className="mb-6 bg-white rounded-xl shadow-sm border border-gray-200 p-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className="text-sm text-gray-600">
              Currently on page{' '}
              <span className="font-semibold text-indigo-600">{pagination.page}</span> of{' '}
